 ...
 * Provides read/write method execution, gas estimation, and result logging.
 *
 * This is NOT a duplicate of MintRedeem — it's a low-level developer tool
 * that exposes all contract methods with raw parameter inputs and console output.
 *
 * Fixes applied:
 * - Fix #2:  getMintQuote returns description corrected to `(uint256 powerAmount, uint256 fee)`
 * - Fix #3:  getRedeemQuote returns description corrected to `(uint256 ltcAmount, uint256 fee)`
 * - Fix #4:  totalCollateral now reads simulated state directly instead of computing from totalSupply/rate
 * - Fix #5:  `hardCap` renamed to `HARD_CAP` to match contract constant name
 * - Fix #6:  Added 5 missing high-value methods: remainingSupply, getContractState, transfer,
 *            getEffectiveMonthlyRate, getEscalatorForYear
 * - Fix #15: Replaced non-standard `w-4.5 h-4.5` with `w-[18px] h-[18px]`
 * - TS Fix:  Added `as const` to CallResult object literals to preserve status literal types
 */
...
const CONTRACT_METHODS: ContractMethod[] = [
  // ─── Read Methods ────────────────────────────────────────────────────────
  {
    ...
    type: 'read',
    selector: '0x18160ddd',
    description: 'Returns the total supply of POWER tokens',
    params: [],
    returns: 'uint256',
  },
  {
    ...
    type: 'read',
    selector: '0x70a08231',
    description: 'Returns the POWER balance of an address',
  ...
  },
  {
    ...
    type: 'read',
    selector: '0x3ba0b9a9',
    description: 'Returns the current LTC → POWER exchange rate',
    params: [],
    returns: 'uint256',
  },
  {
  ...
  },
  {
    ...
    type: 'read',
    selector: '0xfb86a404',
    description: 'Returns the maximum supply cap (84,000,000 POWER)',
    params: [],
    returns: 'uint256',
  },
  {
    ...
    type: 'read',
    selector: '0x8ada066e',
    description: 'Returns remaining POWER tokens that can be minted before hard cap',
    params: [],
    returns: 'uint256',
  },
  {
    ...
    type: 'read',
    selector: '0xa1c5d3f4',
    description: 'Get a quote for minting POWER with a given LTC amount',
    params: [{ name: 'ltcAmount', type: 'uint256', placeholder: '1.0' }],
    returns: '(uint256 powerAmount, uint256 fee)',
  },
  {
    name: 'getRedeemQuote',
    type: 'read',
    selector: '0xb7e04db1',
    description: 'Get a quote for redeeming POWER tokens',
    params: [{ name: 'powerAmount', type: 'uint256', placeholder: '100.0' }],
  ...
  // ─── Write Methods ───────────────────────────────────────────────────────
  {
    ...
    type: 'write',
    selector: '0x1249c58b',
    description: 'Mint POWER tokens by depositing LTC (payable)',
    params: [{ name: 'ltcAmount', type: 'uint256', placeholder: '1.0' }],
  },
  {
    name: 'redeem',
    type: 'write',
    selector: '0xdb006a75',
    description: 'Redeem POWER tokens for LTC',
    params: [{ name: 'powerAmount', type: 'uint256', placeholder: '100.0' }],
  },
  {
    name: 'approve',
    type: 'write',
    selector: '0x095ea7b3',
    description: 'Approve spender to transfer POWER tokens',
    params: [
  ...
  },
  {
    ...
    type: 'write',
    selector: '0xa9059cbb',
    description: 'Transfer POWER tokens to another address',
    params: [
    ...
    try {
      ...
      switch (selectedMethod.name) {
        case 'totalSupply': {
          ...
          result = `${state.totalSupply.toLocaleString()} POWER (${state.totalSupply.toFixed(0)} × 10^18 wei)`;
          break;
        }
        case 'balanceOf': {
          ...
          result = `${bal.toLocaleString(undefined, { maximumFractionDigits: 4 })} POWER`;
          break;
        }
        case 'exchangeRate': {
          const rate = getExchangeRate();
          result = `${rate.toFixed(4)} POWER per LTC (${(rate * 1e18).toExponential(4)} wei)`;
          break;
        }
        case 'protocolFeeBps': {
        ...
        case 'HARD_CAP': {
          result = `${PROTOCOL_CONSTANTS.HARD_CAP.toLocaleString()} POWER (${PROTOCOL_CONSTANTS.HARD_CAP})`;
          break;
        }
        case 'remainingSupply': {
          const state = getEscalatorState();
          const remaining = Math.max(0, PROTOCOL_CONSTANTS.HARD_CAP - state.totalSupply);
          result = `${remaining.toLocaleString(undefined, { maximumFractionDigits: 0 })} POWER (${((remaining / PROTOCOL_CONSTANTS.HARD_CAP) * 100).toFixed(2)}% of hard cap)`;
          break;
        }
        case 'getMintQuote': {
          ...
          result = `powerAmount: ${quote.powerAmount.toLocaleString(undefined, { maximumFractionDigits: 4 })} POWER | fee: ${quote.fee.toFixed(6)} LTC`;
          break;
        }
        case 'getRedeemQuote': {
          const pwrAmt = parseFloat(paramValues['powerAmount'] || '0');
          if (pwrAmt <= 0) throw new Error('POWER amount must be > 0');
          const quote = getRedeemQuote(pwrAmt);
        ...
        case 'getContractState': {
          ...
          result = [
            `totalSupply: ${state.totalSupply.toLocaleString()} POWER`,
            `hardCap: ${PROTOCOL_CONSTANTS.HARD_CAP.toLocaleString()} POWER`,
            `monthlyRate: ${PROTOCOL_CONSTANTS.BASE_MONTHLY_RELEASE.toLocaleString()} POWER`,
        ...
        case 'getEffectiveMonthlyRate': {
          ...
          result = `${effectiveRate.toLocaleString(undefined, { maximumFractionDigits: 2 })} POWER/month (base: ${PROTOCOL_CONSTANTS.BASE_MONTHLY_RELEASE} × escalator: ${escalatorMultiplier.toFixed(4)})`;
          break;
        }
        ...
        case 'redeem': {
          ...
          const pwrAmt = parseFloat(paramValues['powerAmount'] || '0');
          if (pwrAmt <= 0) throw new Error('POWER amount must be > 0');
          if (pwrAmt > powerBalance) throw new Error(`Insufficient POWER balance: ${powerBalance.toFixed(2)}`);
          txHash = await redeemPower(pwrAmt);
        ...
        case 'approve': {
          ...
          result = `Approved ${paramValues['amount'] || '0'} POWER for ${shortenAddress(spender)}`;
          break;
        }
        case 'transfer': {
          if (!address) throw new Error('Wallet not connected');
          ...
          if (transferAmt > powerBalance) throw new Error(`Insufficient POWER balance: ${powerBalance.toFixed(2)}`);
          const sig = await signMessage(`transfer(${to}, ${transferAmt})`);
          txHash = '0x' + sig.slice(2, 66);
          gasUsed = estimateGas(to, '0xa9059cbb');
          result = `Transferred ${transferAmt.toLocaleString()} POWER to ${shortenAddress(to)}`;
          break;
        }
        default:
      ...
      // TS Fix: Explicitly type the status field as the literal to satisfy CallResult['status']
                ...
                {selectedMethod.params.map(param => (
                  <div key={param.name}>
                    ...
                    <input
                      ...
                      className="w-full bg-background/60 border border-white/[0.08] rounded-lg px-3 py-2 text-xs text-white font-mono placeholder:text-textSecondary/30 focus:outline-none focus:border-primary/40 transition-colors"
                    />
                  </div>
                ))}
              </div>
